...
if __name__ == "__main__":
    ...
    #列表共20页，每页有8个项目，每个项目打开评论有page_num页
    for pageNum in range(1, 21):
        ...
        for div in div_list:
            ...
            page_num_list = detail_tree.xpath('//div[@class="pagebox"]/a/text()')   ###关键点！！！
            if len(page_num_list) == 0:
                page_num = 1
            else:
                page_num = detail_tree.xpath('//div[@class="pagebox"]/a/text()')[-2]
            # print(page_num)
            #['2', '3', '4', '5', ' ']
            #['2', '3', '4', '5', '20', ' ']
            #['2', '3', ' ']
            ...
            # page = input("输入一个页码：")
            count = 0
            # 每个景点评论的页数不同，都有第一页，但不一定有第二页第三页，否则就报错了。这里怎么办？
            # 想到把末尾页数变成变量page_num，每次从网页中爬取这个变量
            for page in range(1,int(page_num)+1):  #记得+1！！！
                count = count + 1
                data = {
                    'type': 'all',
                    'currentPage': page,       #？？
